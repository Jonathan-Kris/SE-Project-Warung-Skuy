...
class LoanForm(FlaskForm):
    title = StringField(label="Judul Pinjaman", validators=[DataRequired()])
    tenor = FloatField(label="Tenor", validators=[])
    start_loan = DateField(label="Mulai Pinjaman", validators=[])
    ...
    business_address = StringField(
        label="Alamat atau Lokasi Usaha", validators=[])
    gross_income = IntegerField(
        label="Pendapatan Kotor per Bulan", validators=[])
    net_income = IntegerField(
        label="Pendapatan Bersih per Bulan", validators=[])
    modal = IntegerField(label="Modal Usaha", validators=[])
